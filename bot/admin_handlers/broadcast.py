...
def start_broadcast_flow(call, params):
    ...
    try:
        all_panel_users = combined_handler.get_all_users_combined()
        ...
        online_uuids = {u['uuid'] for u in all_panel_users if u.get('uuid') and u.get('is_active') and u.get('last_online') and isinstance(u.get('last_online'), datetime) and u['last_online'].astimezone(pytz.utc) >= now_utc - timedelta(minutes=3)}
        counts['online'] = len({uuid_to_user_id_map.get(uuid) for uuid in online_uuids if uuid_to_user_id_map.get(uuid)})

        active_1_uuids = {u['uuid'] for u in all_panel_users if u.get('uuid') and u.get('last_online') and isinstance(u.get('last_online'), datetime) and u['last_online'].astimezone(pytz.utc) >= now_utc - timedelta(days=1)}
        counts['active_1'] = len({uuid_to_user_id_map.get(uuid) for uuid in active_1_uuids if uuid_to_user_id_map.get(uuid)})

        inactive_7_uuids = {u['uuid'] for u in all_panel_users if u.get('uuid') and u.get('last_online') and isinstance(u.get('last_online'), datetime) and 1 <= (now_utc - u['last_online'].astimezone(pytz.utc)).days < 7}
        counts['inactive_7'] = len({uuid_to_user_id_map.get(uuid) for uuid in inactive_7_uuids if uuid_to_user_id_map.get(uuid)})
        
        inactive_0_uuids = {u['uuid'] for u in all_panel_users if u.get('uuid') and not u.get('last_online')}
...
def ask_for_broadcast_message(call, params):
    ...
    admin_conversations[uid] = {'broadcast_target': target_group, 'msg_id': msg_id}
    target_names = {
        'all': 'همه کاربران ربات', 'online': 'آنلاین در ۳ دقیقه اخیر', 'active_1': 'فعال در ۲۴ ساعت اخیر',
        'inactive_7': 'غیرفعال در ۷ روز گذشته', 'inactive_0': 'هرگز متصل نشده'
    }
...
def _send_broadcast(message: types.Message):
    ...
    if target_group != 'all':
        ...
        if target_group == 'online':
            ...
            filtered_users = [u for u in all_users if u.get('is_active') and u.get('last_online') and isinstance(u.get('last_online'), datetime) and u['last_online'].astimezone(pytz.utc) >= deadline]
        elif target_group == 'active_1':
            deadline = now_utc - timedelta(days=1)
            filtered_users = [u for u in all_users if u.get('last_online') and isinstance(u.get('last_online'), datetime) and u['last_online'].astimezone(pytz.utc) >= deadline]
        elif target_group == 'inactive_7':
            filtered_users = [u for u in all_users if u.get('last_online') and isinstance(u.get('last_online'), datetime) and 1 <= (now_utc - u['last_online'].astimezone(pytz.utc)).days < 7]
        elif target_group == 'inactive_0':
            filtered_users = [u for u in all_users if not u.get('last_online')]
        
        uuids_to_fetch = [u['uuid'] for u in filtered_users if u.get('uuid')]
    ...
    if not target_user_ids:
        _safe_edit(admin_id, original_msg_id, "هیچ کاربری در گروه هدف یافت نشد\\.", reply_markup=back_to_broadcast_menu)
        try: bot.delete_message(chat_id=admin_id, message_id=message.message_id)
        except Exception: pass
        return

    unique_targets = set(target_user_ids) - {admin_id}
    if not unique_targets:
        _safe_edit(admin_id, original_msg_id, "هیچ کاربری در گروه هدف یافت نشد (یا فقط شما در این گروه بودید)\\.", reply_markup=back_to_broadcast_menu)
        try: bot.delete_message(chat_id=admin_id, message_id=message.message_id)
        except Exception: pass
        return
    ...
    try: bot.delete_message(chat_id=admin_id, message_id=message.message_id)
    except Exception as e: logger.warning(f"Could not delete broadcast source message for admin {admin_id}: {e}")
